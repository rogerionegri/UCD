...
FUNCTION OTSU_THRESHOLD, image, rule, THRESHOLDED_IMAGE=thresholdedImage;, VERBOSE=verbose
...
;-
   ...
   ;h = HISTOGRAM( image - minimumDC, OMIN=minDC, OMAX=maxDC, BINSIZE=(max(image)-min(image))*0.025 )
   ;FredmanDiaconis = 2*IQR(image)*(N_ELEMENTS(image)^(-1.0/3.0))
   if rule eq 0 then BS = 0.5*IQR(image)*(N_ELEMENTS(image)^(-1.0/3.0)) $ ;freedman-diaconis rule
   else BS = 3.49*STDDEV(image)*(N_ELEMENTS(image)^(-1.0/3)) ;scott rule
   
   h = HISTOGRAM( image - minimumDC, OMIN=minDC, OMAX=maxDC, BINSIZE=BS, locations=x )
